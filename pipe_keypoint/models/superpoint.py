...
from .. import PIPE_KEYPOINT_ROOT
...
class SuperPoint(BaseModel):
    ...
    def _init(self, conf):
        ...
        path = PIPE_KEYPOINT_ROOT / 'resources' / 'weights' / 'superpoint_v1.pth'
        if path.exists():
            weights = torch.load(str(path), map_location='cpu')
        else:
            weight_url = "https://github.com/cvg/GlueStick/raw/main/resources/weights/superpoint_v1.pth"
            weights = torch.hub.load_state_dict_from_url(weight_url, map_location='cpu')
    ...
    def _forward(self, data):
        ...
        if self.conf.has_descriptor:
            ...
            if self.conf.max_num_keypoints == 0:
                ...
                return {
                    ...
                    'descriptors': torch.empty(b_size, self.conf.descriptor_dim),
        ...
        if self.conf.sparse_outputs:
            ...
            # Extract keypoints
            keypoints = [torch.nonzero(s > self.conf.detection_threshold) for s in scores]
            scores = [s[tuple(k.t())] for s, k in zip(scores, keypoints)]

            # Discard keypoints near the image borders
            keypoints, scores = list(zip(*[remove_borders(k, s, self.conf.remove_borders, h * 8, w * 8)
                                           for k, s in zip(keypoints, scores)]))

            # Keep the k keypoints with highest score
            if self.conf.max_num_keypoints > 0:
                keypoints, scores = list(zip(*[top_k_keypoints(k, s, self.conf.max_num_keypoints)
                                               for k, s in zip(keypoints, scores)]))
